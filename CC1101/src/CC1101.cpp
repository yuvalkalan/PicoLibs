...
int8_t CC1101::rssi_convert(uint8_t raw_rssi)
{
...
void CC1101::init()
{
    ...
    gpio_init(m_gdo0);
    gpio_set_dir(m_gdo0, GPIO_IN); // Config GDO0 as input
    gpio_init(m_gdo2);
    gpio_set_dir(m_gdo2, GPIO_IN); // Config GDO2 as input
    ...
    uint8_t partnum = read_single_byte(CC1101_PARTNUM | CC1101_READ_SINGLE_BYTE); // reads cc1101 partnumber
    uint8_t version = read_single_byte(CC1101_VERSION | CC1101_READ_SINGLE_BYTE); // reads cc1101 version number
    ...
    Logger::print(LogLevel::TRACE, "set to receive mode!\n");
}

void CC1101::set_myaddr(uint8_t addr)
{
    m_address = addr;
    write_single_byte(CC1101_ADDR | CC1101_WRITE_SINGLE_BYTE, addr); // stores MyAddr in the cc1101
}
void CC1101::set_channel(uint8_t channel)
{
    m_channel = channel;
    write_single_byte(CC1101_CHANNR | CC1101_WRITE_SINGLE_BYTE, channel); // stores the new channel # in the cc1101
}
void CC1101::set_ISM(uint8_t ism_freq)
{
    ...
    case CC1101_FREQ_315MHZ:
        ...
        freq1 = 0x1D;
        freq0 = 0x89;
        write_burst(CC1101_PATABLE_BURST | CC1101_WRITE_BURST, patable_power_315, 8);
        break;
    case CC1101_FREQ_434MHZ:
        freq2 = 0x10;
        freq1 = 0xB0;
        freq0 = 0x71;
        write_burst(CC1101_PATABLE_BURST | CC1101_WRITE_BURST, patable_power_433, 8);
        break;
    case CC1101_FREQ_868MHZ:
        freq2 = 0x21;
        freq1 = 0x65;
        freq0 = 0x6A;
        write_burst(CC1101_PATABLE_BURST | CC1101_WRITE_BURST, patable_power_868, 8);
        break;
    case CC1101_FREQ_915MHZ:
        freq2 = 0x23;
        freq1 = 0x31;
        freq0 = 0x3B;
        write_burst(CC1101_PATABLE_BURST | CC1101_WRITE_BURST, patable_power_915, 8);
        break;
    default: // default is 868.3MHz
        freq2 = 0x21;
        freq1 = 0x65;
        freq0 = 0x6A;
        write_burst(CC1101_PATABLE_BURST | CC1101_WRITE_BURST, patable_power_868, 8); // sets up output power ramp register
        break;
    }
    write_single_byte(CC1101_FREQ2 | CC1101_WRITE_SINGLE_BYTE, freq2); // stores the new freq setting for defined ISM band
    write_single_byte(CC1101_FREQ1 | CC1101_WRITE_SINGLE_BYTE, freq1);
    write_single_byte(CC1101_FREQ0 | CC1101_WRITE_SINGLE_BYTE, freq0);
}
void CC1101::set_mode(uint8_t mode)
{
    ...
    {
    case 0x01:
        write_burst(0 | CC1101_WRITE_BURST, cc1101_GFSK_1_2_kb, CC1101_CFG_REGISTER);
        break;
    case 0x02:
        write_burst(0 | CC1101_WRITE_BURST, cc1101_GFSK_38_4_kb, CC1101_CFG_REGISTER);
        break;
    case 0x03:
        write_burst(0 | CC1101_WRITE_BURST, cc1101_GFSK_100_kb, CC1101_CFG_REGISTER);
        break;
    case 0x04:
        write_burst(0 | CC1101_WRITE_BURST, cc1101_MSK_250_kb, CC1101_CFG_REGISTER);
        break;
    case 0x05:
        write_burst(0 | CC1101_WRITE_BURST, cc1101_MSK_500_kb, CC1101_CFG_REGISTER);
        break;
    case 0x06:
        write_burst(0 | CC1101_WRITE_BURST, cc1101_OOK_4_8_kb, CC1101_CFG_REGISTER);
        break;
    default:
        write_burst(0 | CC1101_WRITE_BURST, cc1101_GFSK_100_kb, CC1101_CFG_REGISTER);
        break;
...
void CC1101::set_output_power_level(int8_t dBm)
{
    for (int i = 0; i < CC1101_PATABLE_SIZE; i++)
    {
        ...
        {
            write_single_byte(CC1101_FREND0 | CC1101_WRITE_SINGLE_BYTE, i);
...
bool CC1101::rx_payload_burst(Packet &packet)
{
    ...
    uint8_t bytes_in_RXFIFO = read_single_byte(CC1101_RXBYTES | CC1101_READ_SINGLE_BYTE);
    // check overflow
    ...
    if (num_bytes > 0)
    {
        read_burst(CC1101_RXFIFO_BURST | CC1101_READ_BURST, (uint8_t *)&packet, num_bytes);
        return true;
    }
...
{
    if (gpio_get(m_gdo2)) // if RF package received
    {
        if (read_single_byte(CC1101_IOCFG2 | CC1101_READ_SINGLE_BYTE) == 0x06) // if sync word detect mode is used
        {
            while (gpio_get(m_gdo2))
...
{
    uint8_t raw_rssi = read_single_byte(CC1101_RSSI | CC1101_READ_SINGLE_BYTE);
...
bool CC1101::send_packet(Packet &packet)
{
    ...
    wait_finish_tx();
    write_burst(CC1101_TXFIFO_BURST | CC1101_WRITE_BURST, (uint8_t *)&packet, packet.header.length + 1); // write data to TX FIFO
    transmit_workmode();                                                                                 // sends data over air
    return true;
}

CC1101::CC1101(spi_inst_t *spi, uint miso, uint csn, uint sck, uint mosi, uint gdo2, uint gdo0, uint8_t freq, uint8_t mode, uint8_t channel, uint8_t address)
    : HighSPI(spi, miso, csn, sck, mosi, CC1101_SPI_BAUDRATE), m_gdo2(gdo2), m_gdo0(gdo0), m_freq(freq), m_mode(mode), m_channel(channel), m_address(address)
{
}
